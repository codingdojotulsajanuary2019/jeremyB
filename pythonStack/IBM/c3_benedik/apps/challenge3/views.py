...
def register(request):
    ...
    try:
        hash_email = bcrypt.hashpw(request.POST.get('email').encode(), bcrypt.gensalt())
...
def send_email(request):
    ...
    user_email = user_info.hash_email
    reset_link = 'localhost:8000/c3/passwordreset/{}'.format(user_email)
    send_mail(
...
def submit_password_reset(request, email):
    try:
        user = User.objects.get(hash_email=email)
    except Exception:
