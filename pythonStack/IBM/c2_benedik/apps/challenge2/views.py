...
def edit(request, uid):
    if not logged_in(request):
        messages.error(request, "Please log in", extra_tags='login')
        return HttpResponseRedirect(reverse('c2:index'))
    if int(request.session['user_id'])== int(uid):
        return render(request, 'challenge2/edit.html', {'uid': uid})
    else:
        messages.error(request, "Feel bad about yourself, ingrate", extra_tags='login')
        return HttpResponseRedirect(reverse('c2:logout'))
