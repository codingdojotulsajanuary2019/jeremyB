...
class ModelManager(models.Manager):
    def register_validator(self, postData):
        ...
        if not User.objects.filter(email=postData['email']):
            ...
            if postData['bday'] == '':
                errors['bday'] = "Please enter a date"
            else:
                thirteen_years = datetime.now() - timedelta(days=13*365)
                print(thirteen_years)
                birthday_check = datetime.strptime(
                    postData['bday'], '%Y-%m-%d')
                print(birthday_check)
                print(birthday_check < thirteen_years)
                if birthday_check > thirteen_years:
        ...
        if not postData['login_email']:
            ...
            if not user_info:
                ...
                user = User.objects.filter(
                    email=postData['login_email']).values()
